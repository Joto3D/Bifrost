...
actor SaveBackup {
    ...
    /// Parses `<yyyyMMdd-HHmmss>-<reason>.zip` back into its date and
    /// reason. The timestamp is a fixed 15 characters, so this looks for
    /// exactly that many characters followed by a "-" rather than
    /// splitting generically — `reason` itself may contain dashes (e.g.
    /// "pre-launch").
    static func parseFilename(_ name: String) -> (date: Date, reason: String)? {
        ...
        let timestampEnd = stem.index(stem.startIndex, offsetBy: 15)
        guard stem[timestampEnd] == "-" else { return nil }
        let timestampPart = String(stem[..<timestampEnd])
        let reason = String(stem[stem.index(after: timestampEnd)...])
        guard let date = timestampFormatter.date(from: timestampPart), !reason.isEmpty else { return nil }
        return (date, reason)
    }
}
